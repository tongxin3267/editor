...
WebInspector.JavaScriptBreakpointsSidebarPane.prototype = {
    _emptyElementContextMenu: function (event) {
        ...
        if (beforeElement)
            this.listElement.insertBefore(element, beforeElement); else {
...
WebInspector.JavaScriptSourceFrame.prototype = {
    _showInfobar: function (infobarElement) {
        ...
        var anchorBox = new AnchorBox(leftCorner.x, leftCorner.y, rightCorner.x - leftCorner.x, leftCorner.height);
        anchorBox.highlight = {lineNumber: lineNumber, startColumn: token.startColumn, endColumn: token.endColumn};
        return anchorBox;
        ...
        if (!executionContext)
            return;
        var rawLocation = (this._uiSourceCode.uiLocationToRawLocation(executionContext.target(), lineNumber, 0));
...
WebInspector.WatchExpressionsSection.prototype = {
    update: function (e) {
        ...
        WebInspector.targetManager.targets().forEach(function (target) {
            target.runtimeAgent().releaseObjectGroup(this._watchObjectGroupId)
        }, this);
        var properties = [];
        var propertyCount = 0;
...
WebInspector.FormatterScriptMapping.prototype = {
    rawLocationToUILocation: function (rawLocation) {
        ...
        if (uiSourceCode.contentType() === WebInspector.resourceTypes.Script) {
            var rawLocation = (uiSourceCode.uiLocationToRawLocation(this._debuggerModel.target(), 0, 0));
        ...
        function innerCallback(formattedContent, formatterMapping) {
            ...
            var formattedUISourceCode = (this._workspace.uiSourceCode(this._projectId, path));
...
WebInspector.SourcesPanel.prototype = {
    targetAdded: function (target) {
        ...
        if (details.reason === WebInspector.DebuggerModel.BreakReason.DOM) {
            ...
            this.sidebarPanes.callstack.setStatus(WebInspector.UIString("Paused on a debugged function")); else {
        ...
        handler = function () {
            return WebInspector.actionRegistry.execute("debugger.toggle-pause");
        };
...
WebInspector.SourcesPanel.TogglePauseActionDelegate.prototype = {
    handleAction: function () {
        (WebInspector.inspectorView.showPanel("sources")).togglePause();
        return true;
    }
}