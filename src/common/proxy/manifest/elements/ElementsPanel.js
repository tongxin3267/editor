...
WebInspector.Spectrum.Events = {ColorChanged: "ColorChanged"};
...
WebInspector.SpectrumPopupHelper.Events = {Hidden: "Hidden"};
...
WebInspector.ElementsTreeOutline.prototype = {
    target: function () {
        ...
        if (element && element._node)
            this._domModel.highlightDOMNodeWithConfig(element._node.id, {
                mode: "all",
                showInfo: !WebInspector.KeyboardShortcut.eventHasCtrlOrMeta(event)
            }); else
...
WebInspector.ElementsTreeElement.ForbiddenClosingTagElements = ["area", "base", "basefont", "br", "canvas", "col", "command", "embed", "frame", "hr", "img", "input", "keygen", "link", "meta", "param", "source"].keySet();
    ...
    highlightSearchResults: function (searchQuery) {
        ...
        if (!this._readyToUpdateSelection) {
            if (document.body.offsetWidth > 0)
                this._readyToUpdateSelection = true; else {
                return;
            }
        }
        ...
        if (tag.getElementsByClassName("webkit-html-attribute").length > 0)
            tag.insertBefore(node, tag.lastChild); else {
        ...
        config.setMultilineOptions(initialValue, {
            name: "xml",
            htmlMode: true
...
WebInspector.MetricsSidebarPane.prototype = {
    update: function (node) {
        ...
        var boxColors = [WebInspector.Color.PageHighlight.Content, WebInspector.Color.PageHighlight.Padding, WebInspector.Color.PageHighlight.Border, WebInspector.Color.PageHighlight.Margin, WebInspector.Color.fromRGBA([0, 0, 0, 0])];
...
WebInspector.RenderingOptionsView.prototype = {__proto__: WebInspector.VBox.prototype};
    ...
    _styleSheetRuleEdited: function (editedRule, oldRange, newRange) {
        ...
        for (var i = 0; i < styles.pseudoElements.length; ++i) {
            ...
            for (var j = pseudoElementCSSRules.rules.length - 1; j >= 0; --j) {
                ...
                styleRules.push({
                    ...
                    rule: rule,
                    editable: !!(rule.style && rule.style.styleSheetId)
                });
            }
            usedProperties = {};
            this._markUsedProperties(styleRules, usedProperties);
        ...
        for (var i = 0; sections && i < sections.length; ++i) {
            ...
            var styleRule = {
                ...
                rule: section.rule,
                editable: !!(section.styleRule.style && section.styleRule.style.styleSheetId),
        ...
        for (var i = 0; i < styleRules.length; ++i) {
            ...
            if (typeof editable === "undefined")
                editable = true;
            if (computedStyle)
                var section = new WebInspector.ComputedStylePropertiesSection(this, styleRule, usedProperties); else {
        ...
        if ((!WebInspector.isMac() && e.keyCode === WebInspector.KeyboardShortcut.Keys.Ctrl.code) || (WebInspector.isMac() && e.keyCode === WebInspector.KeyboardShortcut.Keys.Meta.code)) {
            if (this._elementUnderMouse)
                this._elementUnderMouse.classList.add("styles-panel-hovered");
        }
    }, _keyUp: function (e) {
    ...
    _styleSheetRuleEdited: function (editedRule, oldRange, newRange) {
        ...
        for (var i = styleRule.media.length - 1; i >= 0; --i) {
            ...
            if (media.sourceURL) {
                ...
                var anchor;
                if (rawLocation)
                    anchor = this._parentPane._linkifier.linkifyCSSLocation(mediaHeader.id, rawLocation); else {
...
WebInspector.BlankStylePropertiesSection = function (stylesPane, defaultSelectorText) {
    WebInspector.StylePropertiesSection.call(this, stylesPane, {
...
WebInspector.StylePropertyTreeElementBase.prototype = {
    node: function () {
        ...
        var isEditable = !!(this._styleRule && this._styleRule.editable !== false);
...
WebInspector.StylePropertyTreeElement.prototype = {
    node: function () {
        ...
        if (section && section._parentPane)
            section._parentPane._refreshUpdate(section, false, userCallback); else {
            if (userCallback)
                userCallback();
        }
        ...
        var newStyleRange = (newStyle.range);
        this.style = newStyle;
        ...
        if (isEnterKey(event)) {
            ...
            result = "cancel"; else if (!isEditingName && this._newProperty && event.keyCode === WebInspector.KeyboardShortcut.Keys.Backspace.code) {
...
WebInspector.ElementsPanel.prototype = {
    targetAdded: function (target) {
        ...
        this._documentUpdated((event.target), (event.data));
        ...
        if (listItem && listItem.treeElement && listItem.treeElement.treeOutline instanceof WebInspector.ElementsTreeOutline) {
            var node = (listItem.treeElement.representedObject);
        ...
        var node = (crumbElement ? crumbElement.representedObject : null);
        if (node)
            node.highlight();
        ...
        function handleUndoRedo() {
            ...
            var isRedoKey = WebInspector.isMac() ? event.metaKey && event.shiftKey && event.keyIdentifier === "U+005A" : event.ctrlKey && event.keyIdentifier === "U+0059";
            if (isRedoKey) {
        ...
        return userAgentShadowRoot ? (userAgentShadowRoot.parentNode) : node;
