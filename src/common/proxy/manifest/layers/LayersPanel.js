...
WebInspector.LayerTreeOutline.prototype = {
    selectLayer: function (layer) {
        ...
        for (var node = (this._treeOutline.children[0]); node && !node.root;) {
        ...
        var layer = (node.representedObject);
...
WebInspector.LayerTreeElement.prototype = {
    onattach: function () {
        ...
        var layer = (this.representedObject);
...
WebInspector.LayerDetailsView.prototype = {
    setObject: function (activeObject) {
        ...
        if (event.which !== 1)
            return;
        this.dispatchEventToListeners(WebInspector.LayerDetailsView.Events.ObjectSelected, {
            layer: this._layer,
            scrollRectIndex: index
        });
        ...
        element.textContent = WebInspector.LayerTreeModel.ScrollRectType[scrollRect.type].description + " (" + scrollRect.rect.x + ", " + scrollRect.rect.y + ", " + scrollRect.rect.width + ", " + scrollRect.rect.height + ")";
...
WebInspector.PaintProfilerView.Events = {WindowChanged: "WindowChanged"};
...
WebInspector.LayersPanel.DetailsViewTabs = {Details: "details", Profiler: "profiler"};
...
WebInspector.LayersPanel.LayerTreeRevealer.prototype = {
    reveal: function (snapshotData) {
        if (!(snapshotData instanceof WebInspector.DeferredLayerTree))
            return;
        var panel = (WebInspector.inspectorView.showPanel("layers"));
