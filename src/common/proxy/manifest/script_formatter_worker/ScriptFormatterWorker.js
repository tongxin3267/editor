...
CodeMirror.defineMode("css", function (config, parserConfig) {
    ...
    return {
        startState: function (base) {
            return {tokenize: tokenBase, baseIndent: base || 0, stack: [], lastToken: null};
            ...
            if (type == "{") {
                ...
                state.stack[state.stack.length - 1] = "@media"; else if (type == "(") {
                if (context == "@media" || context == "@mediaType") {
...
;CodeMirror.defineMode("htmlmixed", function (config, parserConfig) {
    ...
    scriptTypes.push({matches: /./, mode: CodeMirror.getMode(config, "text/plain")});
        ...
        switch (state) {
            case FormatterWorker.CSSParserStates.Initial:
                if (tokenType["qualifier"] || tokenType["builtin"] || tokenType["tag"]) {
                    rule = {selectorText: tokenValue, lineNumber: lineNumber, columNumber: column, properties: [],};
                    state = FormatterWorker.CSSParserStates.Selector;
                } else if (tokenType["def"]) {
                    rule = {atRule: tokenValue, lineNumber: lineNumber, columNumber: column,};
            ...
            case FormatterWorker.CSSParserStates.Style:
                if (tokenType["meta"] || tokenType["property"]) {
                    property = {name: tokenValue, value: "",};
...
function tokenizer($TEXT) {
    ...
    function token(type, value, is_comment) {
        ...
        var ret = {type: type, value: value, line: S.tokline, col: S.tokcol, pos: S.tokpos, nlb: S.newline_before};
        if (!is_comment) {
...
FormatterWorker.JavaScriptFormatter.prototype = {
    format: function () {
        ...
        var token = this._peek();
        if (token === FormatterWorker.JavaScriptTokens.IDENTIFIER || token === FormatterWorker.JavaScriptTokens.NUMBER || token === FormatterWorker.JavaScriptTokens.STRING || FormatterWorker.JavaScriptTokens.DELETE <= token && token <= FormatterWorker.JavaScriptTokens.FALSE_LITERAL || token === FormatterWorker.JavaScriptTokens.INSTANCEOF || token === FormatterWorker.JavaScriptTokens.IN || token === FormatterWorker.JavaScriptTokens.CONST) {
            this._next();
            this._parseFunctionLiteral();
        }
